...
class VideoFix extends openfl.display.Bitmap
{
	...
	@:noCompletion
	private static final MAX_AUDIO_BUFFER_COUNT:Int = DefineMacro.getInt('HXVLC_MAX_AUDIO_BUFFER_COUNT', 16);
	#end
	...
	public static var waitForContext3DBeforePlay:Bool = true;

	public static function prewarm(stage:openfl.display.Stage):Void
	{
		if (stage == null)
			return;

		@:nullSafety(Off)
		if (stage.context3D == null && stage.stage3Ds != null && stage.stage3Ds.length > 0)
	...
	public var updateWhenInvisible:Bool = true;
	...
	// LibVLC matches listeners by opaque pointer identity. Keep the exact
	// pinned address used at attach time for every callback and detach call.
	@:noCompletion
	...
	@:noCompletion
	private var texturePlanesBytes:Null<Bytes>;
	...
	public function new(smoothing:Bool = true):Void
	{
		...
		{
			@:privateAccess
			if (Lib.current.stage.context3D == null && Lib.current.stage.stage3Ds != null && Lib.current.stage.stage3Ds.length > 0)
				Lib.current.stage.stage3Ds[0].requestContext3D();

			Lib.current.stage.addEventListener(openfl.events.Event.CONTEXT3D_CREATE, onContext3DCreate);
		}
	}
	...
	private function retainNativeCallbackPin():RawPointer<cpp.Void>
	{
		...
		#else
		// Stock hxcpp objects do not move, so LibVLC can retain the original
		// object identity exactly as it did before the NovaGC backend.
...
private function onContext3DCreate(e:openfl.events.Event):Void
{
    ...
    @:nullSafety(Off)
    if (Lib.current != null && Lib.current.stage != null)
        Lib.current.stage.removeEventListener(openfl.events.Event.CONTEXT3D_CREATE, onContext3DCreate);

    textureMutex.acquire();

		@:nullSafety(Off)
		if (bitmapData != null && bitmapData.__texture == null && Lib.current.stage != null && Lib.current.stage.context3D != null)
		{
			@:privateAccess
	...
	public function startPlay():Bool
	{
		...
		if (useTexture && waitForContext3DBeforePlay)
		{
			@:nullSafety(Off)
			if (Lib.current != null && Lib.current.stage != null && Lib.current.stage.context3D == null)
			{
				@:privateAccess
				if (Lib.current.stage.stage3Ds != null && Lib.current.stage.stage3Ds.length > 0)
					Lib.current.stage.stage3Ds[0].requestContext3D();

				Lib.current.stage.addEventListener(openfl.events.Event.CONTEXT3D_CREATE, onContext3DCreateStartPlay);

				return true;
			}
		}

		return LibVLC.media_player_play(mediaPlayer.raw) == 0;
	}

	@:noCompletion
	private function onContext3DCreateStartPlay(e:openfl.events.Event):Void
	{
		@:nullSafety(Off)
		if (Lib.current != null && Lib.current.stage != null)
			Lib.current.stage.removeEventListener(openfl.events.Event.CONTEXT3D_CREATE, onContext3DCreateStartPlay);

		if (mediaPlayer != null)
			LibVLC.media_player_play(mediaPlayer.raw);
	}
	...
	@:noDebug
	@:unreflective
	private function preparseAndPreallocate():Void
	{
		if (mediaPlayer == null)
			return;

		if (!parseWithOptions(0, 0))
			return;
		...
		var w:UInt32 = 0;
		var h:UInt32 = 0;
		var pw:Pointer<UInt32> = Pointer.addressOf(w);
		var ph:Pointer<UInt32> = Pointer.addressOf(h);

		if (!calculateVideoSize(pw, ph))
		{
			textureMutex.release();
			return;
		}
		...
		if (texturePlanesBytes == null)
			texturePlanesBytes = Bytes.ofData(texturePlanes);

		if (texturePlanesArray == null)
			texturePlanesArray = UInt8Array.fromBytes(texturePlanesBytes);
	...
	{
		if (__renderable || forceRendering || updateWhenInvisible)
		{
			MainLoop.runInMainThread(function():Void
			{
				if (isValid() && texturePlanes != null)
				{
                    ...
                    if (bitmapData != null)
                    {
                        ...
                        if (bitmapData.__texture != null)
                            cast(bitmapData.__texture, VideoTexture).uploadFromTypedArray(texturePlanesArray);
	...
	private function audioPlay(samples:RawPointer<UInt8>, count:UInt32, pts:Int64):Void
	{
		...
		if (alSource != null && alBufferPool != null)
		{
			...
			final alBuffer:Null<ALBuffer> = alBufferPool.pop();
	...
	private function detachEvents():Void
	{
		...
		final attachedEventTypes:Array<Int> = _attachedEventTypes;
		// libvlc_event_detach aborts when the listener is already absent. Publish
		// the empty state first so re-entrant teardown cannot detach it twice.
		_attachedEventTypes = [];

		for (type in attachedEventTypes)
			LibVLC.event_detach(eventManager.raw, type, untyped __cpp__('event_manager_callbacks'), cast _novaNativeCallbackContext);
	}
